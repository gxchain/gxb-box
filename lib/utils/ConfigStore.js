...
export default{
    init() {
        ...
        return new Promise((resolve, reject)=> {
            try{
                self.config = {};
                fs.exists(path.resolve(process.cwd(),'./config/common-config.json'), function(exists) {
                    if (exists) {
                        self.config.common = JSON.parse(fs.readFileSync(path.resolve(process.cwd(),'./config/common-config.json'),'utf-8'));
                    }
                });
                fs.exists(path.resolve(process.cwd(),'./config/merchant-config.json'), function(exists) {
                    if (exists) {
                        self.config.merchant = JSON.parse(fs.readFileSync(path.resolve(process.cwd(),'./config/merchant-config.json'),'utf-8'));
                    }
                });
                fs.exists(path.resolve(process.cwd(),'./config/datasource-config.json'), function(exists) {
                    if (exists) {
