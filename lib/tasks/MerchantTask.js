...
import {ChainTypes, key, PrivateKey} from 'gxbjs';
import Promise from 'bluebird';
import {
    DATA_SOURCE_STATUS_MAP, DOWNLOAD_STATUS, PAY_STATUS, SYSTEM_ERROR_CODE,
...
export default {
    ...
    deal_with_data_transaction (data_transaction) {
        ...
        if (next_request_status == TRANSACTION_STATUS_MAP.CONFIRMED) {
            ...
            next_datasource_status.forEach((item) => {
                ...
                if (status == DATA_SOURCE_STATUS_MAP.UPLOADED) {
                    ...
                    //     }
                    // })
                } else if (status == DATA_SOURCE_STATUS_MAP.PAYED) { // 支付已确认, 则可以下载数据
        ...
        } else if (next_request_status == TRANSACTION_STATUS_MAP.PRIVACY_REJECTED) { // 处理被拒绝的交易
