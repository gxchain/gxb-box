...
import {ChainTypes, hash, key, PrivateKey} from 'gxbjs';
import ConfigStore from '../utils/ConfigStore';

let league_data_product_id_reg = new RegExp(`^1\.${ChainTypes.object_type.league_data_product}\.\\d+$`); // eslint-disable-line
let taskQueue = [];
...
export default {
    ...
    deal_with_data_transaction (data_transaction) {
        ...
        if (!(status && status == TRANSACTION_STATUS_MAP.CONFIRMED)) {
            return;
        } else if (!is_product_subscribed) { // 不处理未订阅的product_id
            return;
        } else {
            GXChainService.fetch_account(config.datasource.account_name).then(function (account) {
                ...
                if (!currentAccountStatus) {
                    return;
                } else if (currentAccountStatus.status > 0) { // 如果数据源状态不是初始化状态,则不处理
                    return;
                } else if (account_id == data_transaction.requester) { // 不处理自己的请求
                    return;
                }
                // 不重复处理同一个请求
                ...
                console.log('开始处理:', data_transaction.request_id, JSON.stringify(data_transaction), ' ', new Date().getTime(), '\n产品id:', data_transaction.product_id, '\n请求方Id:', data_transaction.requester);
