...
<script>
    ...
    export default {
        ...
        created (){
            let commonSettings =  JSON.parse(localStorage.getItem('__gxbBox__commonSettings'));
            if (commonSettings && commonSettings.port && commonSettings.witnesses && commonSettings.faucet_url) {
                this.api_list = commonSettings.witnesses;
                this.formValidate.port = commonSettings.port;
                this.formValidate.faucet_url = commonSettings.faucet_url;
            }
        },
        methods: {
            handleAdd() {
                ...
                this.$refs['formValidate'].validate((valid) => {
                    if (valid) {
                        if (this.api_list.length > 0){
                            commonSettings.port = this.formValidate.port;
                            commonSettings.faucet_url = this.formValidate.faucet_url;
                            commonSettings.witnesses = this.api_list;
                            localStorage.setItem('__gxbBox__commonSettings',JSON.stringify(commonSettings));
                            this.$http({
                                method: 'post',
